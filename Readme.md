...
* Dây kết nối.
* Có thể sử dụng USB–UART thay cho Arduino trong giai đoạn đọc tín hiệu.
...
### 3.1. Kết nối tín hiệu LiDAR với Arduino

| LiDAR | Arduino Uno               |
| ----- | ------------------------- |
| `U`   | 5 V ổn định               |
| `G`   | GND                       |
| `T`   | D8 – RX của AltSoftSerial |

LiDAR chỉ truyền dữ liệu qua chân `T`, do đó chân này phải được nối với chân RX của Arduino.

### 3.2. Kết nối động cơ với L298N
...
Hai dây động cơ LiDAR được nối với đầu ra của L298N:

```text
LiDAR motor (+) ─── OUT1 của L298N
LiDAR motor (-) ─── OUT2 của L298N
```
...
Tất cả GND của Arduino, LiDAR, L298N và nguồn ngoài phải được nối chung.

---

## 4. Thông số truyền UART
...
Máy tính nhận dữ liệu từ Arduino ở tốc độ cao hơn:

```text
Arduino ↔ Máy tính: 230400 bit/s
```

Việc sử dụng tốc độ 230400 bit/s giúp Arduino chuyển tiếp luồng dữ liệu LiDAR 115200 bit/s mà không làm đầy bộ đệm UART.

---

## 5. Điều khiển động cơ LiDAR

Tốc độ động cơ được điều khiển bằng PWM tại chân ENA của L298N.

Ví dụ:

```cpp
const uint8_t MOTOR_ENA = 5;
...
Trong quá trình thử nghiệm, LiDAR bắt đầu phát dữ liệu góc và khoảng cách ổn định hơn khi giảm tốc độ quay của động cơ. Tốc độ làm việc quan sát được nằm trong khoảng:

```text
390–450 RPM
```

Một số thời điểm tốc độ ổn định ở khoảng:

```text
396–402 RPM
```

Giá trị PWM phù hợp phụ thuộc vào:

* Điện áp nguồn động cơ.
* Tổn hao trên L298N.
* Ma sát của cơ cấu quay.
* Dòng tải của động cơ.
* Tốc độ xử lý và truyền dữ liệu của Arduino.

Không nên tăng PWM quá cao ngay từ đầu. Có thể bắt đầu từ khoảng 80–100 và tăng dần đến khi dữ liệu xuất hiện ổn định.

---

## 6. Đọc dữ liệu UART thô bằng Arduino

Thư viện `AltSoftSerial` được sử dụng để nhận dữ liệu từ LiDAR. Trên Arduino Uno, chân RX mặc định của AltSoftSerial là D8.

```cpp
#include <AltSoftSerial.h>

AltSoftSerial lidarSerial;
...
void loop()
{
    while (lidarSerial.available() > 0)
    {
        ...
        // Chuyển tiếp nguyên byte sang máy tính
        Serial.write(data);
    }
}
```
...
`Serial.write()` gửi nguyên byte nhị phân nhận được từ LiDAR. Trong khi đó, `Serial.print()` chuyển byte thành chuỗi ký tự ASCII, làm thay đổi hoàn toàn cấu trúc dữ liệu.

Khi chạy chương trình Python đọc cổng COM, cần đóng Serial Monitor và Serial Plotter của Arduino IDE. Một cổng COM thông thường chỉ có thể được một chương trình sử dụng tại cùng một thời điểm.

---

## 7. Đọc dữ liệu trên máy tính bằng Python
...
print(f"Đang đọc dữ liệu tại {SERIAL_PORT}...")
...
Trên Linux, cổng nối tiếp có thể có dạng:
...
### 11.3. Phân bố góc của các điểm đo
...
## 13. Kiểm tra checksum

Checksum của frame là tổng 16 bit của tất cả byte đứng trước hai byte checksum:

```text
checksum = sum(frame_without_checksum) & 0xFFFF
```

Checksum được lưu theo thứ tự big-endian:
...
Tổng các byte trước checksum:

```text
AA + 00 + 09 + 01 + 61 + AE + 00 + 01 + A2
= 0x0266
```

Hai byte checksum cuối là:

```text
02 66
```

Như vậy frame hợp lệ.
...
## 19. Những nội dung cần tiếp tục kiểm chứng
...
6. Quan hệ giữa trường length ở byte 1–2 và tổng chiều dài frame.
7. Ảnh hưởng của tốc độ quay đến số điểm và tần số lấy mẫu.
8. Độ chính xác khoảng cách tại các bề mặt có độ phản xạ khác nhau.
...
Quá trình nghiên cứu cho thấy có thể đọc và giải mã dữ liệu từ LiDAR ngay cả khi không có tài liệu giao thức chính thức. Phương pháp thực hiện dựa trên việc thu dữ liệu UART thô, quan sát sự lặp lại của frame, thay đổi có kiểm soát tốc độ quay và khoảng cách vật cản, sau đó đối chiếu các trường dữ liệu.
